...
if ( is_seo_date_update() && //検索エンジンに更新日を伝える場合
    ...
    <?php if ( is_create_date_visible() ): //投稿日を表示する場合?>
      <span class="post-date"><span class="fa fa-clock-o fa-fw"></span><time class="entry-date date published"><?php the_time( get_theme_text_date_format() ) ;?></time><?php echo $human_time_diff; ?></span>
...
<?php else: //検索エンジンに投稿日を伝える場合?>
  <?php if ( is_create_date_visible() ): //投稿日を表示する場合?>
    <span class="post-date"><span class="fa fa-clock-o fa-fw"></span><time class="entry-date date published" datetime="<?php echo get_the_time('c') ;?>"><?php the_time( get_theme_text_date_format() ) ;?></time><?php echo $human_time_diff; ?></span>
  <?php endif; //is_create_date_visible?>
  <?php if ( is_update_date_visible() && //更新日を表示する場合
             get_mtime('c') ) : //更新日があるどき?>
    <span class="post-update"><span class="fa fa-history fa-fw"></span><time class="entry-date date updated"><?php if ($mtime = get_mtime( get_theme_text_date_format() )) echo $mtime; ?></time></span>
  <?php endif; //is_update_date_visible?>
<?php endif; ?>
