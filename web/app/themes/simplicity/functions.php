...
//      $post = $posts[0];
      ...
      //はてブを表示する場合
      $hatebu_tag = is_blog_card_hatena_visible() ? '<div class="blog-card-hatebu"><a href="http://b.hatena.ne.jp/entry/'.$url.'"'.$target.'><img src="http://b.hatena.ne.jp/entry/image/'.$url.'" alt="はてブ数" /></a></div>' : '';
      //サイトロゴを表示する場合
      $favicon_tag = '';
...
function simplicity_wp_title( $title, $sep ) {
  ...
  // if($paged >= 2 || $page >= 2)://ページ番号を加える
  //   $title = $title.' - '.sprintf('%sページ',
  //     max($paged,$page));
  // endif;
...
add_filter( 'wp_title', 'simplicity_wp_title', 10, 2 );